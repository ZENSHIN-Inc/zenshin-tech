...
- 社内会議が一つの基盤で完結するなら、**その基盤の標準AI**
- 顧客主催・対面・fileが混在するなら、**Fireflies**
- 日本語・二言語・対面を重視するなら、**Notta**
- 会議横断検索・引用・MCPを重視するなら、**Fathom**
- 営業・CS・VoC分析は**tl;dv**、英語圏・リアルタイム支援は**Otter**
...
<p class="lead-in">自社の会議環境を次のフローに当てはめて、候補を絞り込みます。</p>
...
<p class="lead-in">社内会議が一つの基盤で完結するなら、追加SaaSより先に標準AIを検討します。</p>
...
<p class="lead-in">会議経路が混在する環境で、独立系を1つだけ選ぶ場合の第一候補です。</p>
...
<p class="lead-in">会議の記録そのものより、記録した資産の検索・再利用を重視する場合の候補です。</p>
...
<p class="lead-in">日本語の精度と、二言語が混ざる会議への対応を最優先する場合の候補です。</p>
...
<p class="lead-in">用途が営業・CSの分析か、会議中の支援かで特化製品を選び分けます。</p>
...
<p class="lead-in">どの経路の会議を記録できるかが、製品選定の最初の比較軸です。</p>
...
<p class="note">表内の※はベータ・段階展開・plan・OS差のある条件付き対応。導入環境で利用可否を確認する（2026年7月13日確認）</p>
...
<p class="lead-in">記録した会議をどこまで活用できるかを、横断分析とMCP対応で比べます。</p>
...
<p class="lead-in">ChatGPT / ClaudeからのMCP連携で、できることとできないことを整理します。</p>
...
<p class="lead-in">公称の対応言語数を精度順位と見なさず、実際の日本語会議で次の5点を確認します。</p>
...
<p class="lead-in">導入判断の前に、情報システム部門の統制要件を3つの観点で確認します。</p>

<div class="card">

### 🔑 ID・権限と監査

- SSO / SCIM / role / OAuth scopeを既存のID基盤へ揃えられるか
- audit log・共有履歴・管理者権限で利用状況を追跡できるか
...
- retention / export / 退職者対応を自社の保持方針に合わせられるか
- 保存地域・学習利用の有無・暗号化の仕様を確認できるか
...
- Bot参加・Botなし録音・対面録音それぞれの通知方法があるか
- **共有accountを禁止**し、利用者単位・最小権限で運用できるか
...
<p class="lead-in">見積もりでは、次の5点を掛け合わせた実質費用で比較します。</p>
...
![w:960](../assets/slides/meeting-ai-selection-japan/fireflies-pricing.png)

契約前に公式Pricingページで**最新の価格・課金条件**を必ず確認する。

<p class="note">出典: Fireflies Pricing（fireflies.ai/pricing、2026年7月14日取得）。年払い時のper seat価格</p>
...
<p class="lead-in">標準AI各製品の機能・提供条件は、次の公式資料で確認しました。</p>
...
<p class="lead-in">独立系各製品の機能・料金は、次の公式資料で確認しました。</p>
