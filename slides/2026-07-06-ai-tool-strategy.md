...
***2026年7月現在、主力は GPT-5.6 系と Codex です。***

| 位置づけ | モデル系列 | CLI ツール |
| --- | --- | --- |
| **現在の主力** | GPT-5.6 系 | **Codex** |
| **条件付き併用** | Claude 系 | **Claude Code** |

- **まず GPT-5.6 系を使う**。Sol / Terra / Luna の選択肢が強い
- Claude は **Fable 5 の利用枠が十分なら有力**。Opus 4.8 以下では GPT 系を優先
- 投資先は OpenAI / Anthropic の 2 社だが、***現時点のおすすめは Codex***
...
出典: [Artificial Analysis](https://artificialanalysis.ai/)（2026年7月10日時点、代表構成）

> ***最高性能のSol — 本命のTerra — 高速処理のLuna***
...
「独自 AI」に見えるツールも、中身は ***主要モデル + 指示文と UI*** がほとんど。

---

# 特化ツールとの付き合い方 — V0 の例

**要所で手軽な特化ツールが有効な場面はある。**
...
Claude と OpenAI はこの「**ツールを使わせる仕組み**」の作り込みが頭一つ抜けており、追走勢との差はまだ大きい。

---

# 「Google 製品なら Gemini」は誤解

「Office と連携するなら Copilot」も同じ誤解です。

- Gmail・カレンダー・スプレッドシート・Excel などとの連携は、
  **MCP という標準規格**で公開されている
...
**CLI** = コマンドラインインターフェース。ターミナルで動くツール形式のこと。
...
- **スキル**（定型手順をまとめたプロンプトファイル）で業務を定型化する
- ローカルのスクリプトや MCP と組み合わせ、**業務専用の処理**を組む
...
Claude と OpenAI は**ツール活用のリードカンパニー**。
...
| --- | --- |
| **現在の主力** | OpenAI **$200** |
| Claude の運用知識を維持 | Anthropic $20 |
| Fable 5 の利用枠が十分 | Anthropic **$200** |

- 基本は **OpenAI $200 + Anthropic $20**。Fable 5 を使えるなら **$200 + $200**
...
- 一歩後ろを **Grok 4.5・GLM-5.2・Gemini・Qwen** などの
  **追走モデル**が追う — **約 1 年で今のトップ水準に到達**する
- いま 2 社で貯めるノウハウは、
...
2026年7月現在、
***まず Codex で GPT-5.6 系を使うのが最善手***です。
