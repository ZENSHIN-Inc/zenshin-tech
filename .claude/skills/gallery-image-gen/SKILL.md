...
description: >
  gpt-image-2（Codex CLI の組み込み image_gen・ChatGPT サブスクルート）で画像を生成し、
...
> **Core Principle**: 画像生成は **Codex CLI 経由の組み込み `image_gen`（gpt-image-2）** で行い、**ChatGPT サブスク内で完結させる（OpenAI API の従量課金ルートに入らない）**。生成した画像は `gallery/<YYYYMM>-<slug>/` に置き、main に push されると https://contents.zenshin-inc.co.jp/gallery/ で公開される。
...
- **push = 社外公開**。commit / push はユーザーの明示指示があるまでしない（グローバルルール）。push 前に「公開されます」を一言添える
