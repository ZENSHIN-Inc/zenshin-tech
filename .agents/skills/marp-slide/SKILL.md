...
## このリポジトリ（zenshin-contents）での運用上の注意（ローカル追記）

> 出所: https://github.com/softaworks/agent-toolkit の `skills/marp-slide`（MIT License, commit 3027f20）。本節のみ ZENSHIN による追記で、上記までが原文。

- 公開デッキ（`slides/<YYYY-MM-DD>-<slug>.md`）は **ZENSHIN ブランドテーマ（`theme: zenshin` / `themes/zenshin.css`）を使う**。本スキル同梱の 7 テーマ・テンプレートは社外公開デッキには適用しない（ブランド統一のため。社内ラフや実験用は自由）
- 本スキルは主に **`references/` 配下（Marp 構文・画像レイアウトパターン・ベストプラクティス・テーマ CSS 設計）の参照資料**として使う
- 出力先はこのリポジトリの規約に従う: `/mnt/user-data/outputs/` ではなく `slides/` に置き、`bun run build` → `dist/slides/*.pdf` で確認する
- 挿絵の生成・差し込みは姉妹スキル `gallery-image-gen` / `slide-infographic` を使う。**言葉だけでは分かりにくい概念のページには挿絵を積極的に入れる**（この会社の好みは「挿絵多め」）
- **zenshin テーマ v2.1 の書き方ルール**（テーマ側の部品を使い、デッキ内に自前 CSS を書かない）:
  - **結論・メッセージ主体のページは `<!-- _class: center -->` で中央揃え**にする（「結論から」「まとめ」「推奨プラン」など）。箇条書き中心のページは左揃えのまま。center ページ内でも箇条書き・コールアウトは自動で左揃えが保たれる
  - **見出しに下線・短いバーなどの装飾を付けない**（テーマからも v2.1 で廃止済み。生成 AI スライドの定番装飾に見えるため）
  - **コールアウトを意味に応じて色分けして積極的に使う**: `>`（blockquote）= POINT（ゴールド・要点）／`<div class="callout good">` = GOOD（緑・良い点）／`.warn` = CAUTION（黄・注意）／`.danger` = WARNING（赤・危険）／`.info` = INFO（青・補足）。div 内に Markdown を書くときは開始タグ・終了タグの前後に空行を挟む。ラベル文字列は `data-label="..."` で上書き可
  - 表の直後の余白はテーマが確保する（`section table` に margin-bottom あり。手動の空行調整は不要）
  - **「A → B」「用語 — 説明」を 1 行に詰めた箇条書きにしない**: 対応関係の列挙（例:「コード → 文書作成」）は**表**にし、用語＋説明の列挙（例:「スキル — 特定業務の…」）は**表**か **2 階層の箇条書き**（親 = 用語、子 = 説明）に分解する。1 行詰め込みは折り返しで読みにくくなる
  - **1 ページの文章量予算を守る**: 見出し +（表 or 箇条書き）+ コールアウト 1 個まで。導入文は原則書かない。`bg right` で挿絵を入れるページは特に厳しく「箇条書き 4 個 + コールアウト 3 行」程度が上限（詳細は slide-infographic の「はみ出しの予防」）
  - **はみ出しは `bun run check` で機械検知**（`bun run ci` に組み込み済み）。全ページの PDF 目視は不要で、目視は挿絵入りページの見た目確認だけに絞る
