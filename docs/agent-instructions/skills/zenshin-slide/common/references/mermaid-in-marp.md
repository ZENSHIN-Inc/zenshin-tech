...
```markdown
![w:1120](../assets/slides/<デッキslug>/diagram.svg)
```

## 16:9スライド向けの設計

- 配色はMermaidの`default`テーマ、背景は白を起点にする。スライドの紙色やブランド配色へ合わせすぎるとノードの塗りと背景が同化するため、図のコントラストを優先する
- 白背景には外枠・角丸・影を付けず、図の内部構造だけを見せる
- `flowchart.diagramPadding`を40〜64px程度から調整し、白背景の四辺と端のノードの間に余白を確保する
- Mermaidは明示依頼がある場合だけでなく、表や箇条書きより関係性、順序、分岐を早く理解できる場合に使う
- 表が続くデッキでは、選び方や全体像を示す要所へ図を置き、理解の入口と視覚的なリズムを作る
- 数値や機能の有無など、正確な比較・検索が目的なら表を残す。同じ情報を表と図で重複させない
- Yes / Noフローは原則`flowchart LR`にして、見出し下へ横長に収める
- 質問は5個程度までとし、1ノードを2行以内にする
- 分岐ラベルは`はい`／`いいえ`で統一する
- 製品名などの到達点は質問ノードと形を分け、独自配色は必要最小限にする
